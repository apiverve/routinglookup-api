// Package routinglookup provides a Go client for the Routing Number Lookup API.
//
// For more information, visit: https://apiverve.com/marketplace/routinglookup?utm_source=go&utm_medium=readme
package routinglookup
...
// Request contains the parameters for the Routing Number Lookup API.
//
// Parameters:
//   - routing (required): string - The routing number to lookup information about [minLength: 9, maxLength: 9]
type Request struct {
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		"routing": {Type: "string", Required: true, MinLength: intPtr(9), MaxLength: intPtr(9)},
	}
...
// ResponseData contains the data returned by the Routing Number Lookup API.
type ResponseData struct {
	RoutingNumber string `json:"routing_number"`
	OfficeCode string `json:"office_code"`
	RecordTypeCode string `json:"record_type_code"`
	NewRoutingNumber string `json:"new_routing_number"`
	Bank string `json:"bank"`
	Address string `json:"address"`
	City string `json:"city"`
	State string `json:"state"`
	Zip string `json:"zip"`
	ZipExtension string `json:"zip_extension"`
